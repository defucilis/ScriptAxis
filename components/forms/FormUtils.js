            ...
            <textarea
                ...
                onInput={() => {
                    textArea.current.style.setProperty(
                        "height",
                        getNewHeight(textArea.current.scrollHeight)
                    );
            ...
            <select {...props} value={props.value}>
                {!props.options
                    ...
                    : props.options.map(option => {
                          return (
                              <option key={props.name + "_" + option.value} value={option.value}>
                                  {option.label}
                              </option>
                          );
                      })}
    ...
    return (
        <div>
            ...
            <TagifyTags
                ...
                onBlur={e => {
                    props.onChange({
                        target: { id: props.id, value: e.detail.tagify.state.inputText },
                    });
                }}
            />
            {props.error ? <aside>{props.error}</aside> : null}
...
const Tags = props => {
    ...
    return (
        <div>
            <label htmlFor={props.name}>{props.label}</label>
            <TagifyTags {...props} {...props.tagProps} value={value} onChange={handleChange} />
            {props.error ? <aside>{props.error}</aside> : null}
        </div>
    );
};
...
const InnerDropzone = ({
    ...
    const handleFiles = newFiles => {
        ...
        setFileLabels(
            newFiles.map(file => {
                return (
                    <li key={`${fieldName}_${file.name}`}>
                        {`${file.name} (${(
                            file.size / (file.size > 1000000 ? 1000000 : 1000)
                        ).toFixed(1)} ${file.size > 1000000 ? "MB" : "kB"})`}
                    </li>
                );
            })
        );
    ...
    return (
        <div {...getRootProps({ className: `${className} ${over ? hoveringClassName : null}` })}>
            <input {...getInputProps()} />
            {files.length === 0 ? (
                <p>{instruction || "Drag + drop and image, or click to select one"}</p>
            ) : (
                <ul>{fileLabels}</ul>
            )}
        </div>
    ...
    return (
        <div>
            <p onClick={() => setCollapsed(cur => !cur)}>
                {collapsed ? (
                    <BsCaretRightFill style={iconStyle} />
                ) : (
                    <BsCaretDownFill style={iconStyle} />
                )}
                {props.label}
            </p>
